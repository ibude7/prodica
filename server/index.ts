...
app.post('/v1/products/lookup', async (req, res) => {
  ...
  try {
    let product: ProductResult | null = lookupProduct(parsed)
    if (!product && parsed.kind === 'barcode') {
      product = await lookupOpenFoodFactsByBarcode(parsed.code)
    }
    if (!product && parsed.kind === 'ocr') {
      product = await searchOpenFoodFactsByText(parsed.text)
    }
    res.json({ product })
  } catch (e) {
    console.error(e)
